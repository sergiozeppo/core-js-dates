...
function getNextFriday(date) {
  ...
  let daysToFriday = (5 - now + 7) % 7;
  if (daysToFriday === 0) daysToFriday = 7;
  const nextWeekFriday = new Date(date);
  nextWeekFriday.setDate(date.getDate() + daysToFriday);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diffDays = Math.round(diff / (1000 * 3600 * 24)) + 1;
  return diffDays;
}
...
function isDateInPeriod(date, period) {
  const fromDate = new Date(period.start);
  const toDate = new Date(period.end);
  const d = new Date(date);
...
function formatDate(date) {
  ...
  hours %= 12;
  if (!hours) hours = 12;
  return `${d.getMonth() + 1}/${d.getUTCDate()}/${d.getFullYear()}, ${hours}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')} ${ampm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= days; i += 1) {
    ...
    if (day.getDay() === 0 || day.getDay() === 6) result += 1;
  }
  return result;
}
...
function getWeekNumberByDate(date) {
  ...
  const fixDay = d.getDay() === 0 && date.getDay() !== 0 ? 1 : 0;
  const wholeDay = 1000 * 60 * 60 * 24;
  const daysFrom = Math.ceil((d.getDay() + (date - d) / wholeDay) / 7);
  return daysFrom + fixDay;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (dateStart.getTime() <= dateEnd.getTime()) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (dateStart.getTime() > dateEnd.getTime()) break;
      const workDate = `${String(dateStart.getDate()).padStart(2, '0')}-${String(dateStart.getMonth() + 1).padStart(2, '0')}-${dateStart.getFullYear()}`;
      workSchedule.push(workDate);
